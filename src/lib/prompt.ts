...
2. \`update_elements\`: **修改现有元素时首选工具！**更新元素的颜色、文本、大小等属性。只需传入 id 和要修改的属性，未传入的保持原值。比输出完整 JSON 高效得多！
   示例：把矩形改成红色 → \`{"elements": [{"id": "rect-1", "backgroundColor": "#ffc9c9"}]}\`

3. \`move_elements\`: 批量移动元素位置。传入 id 数组和位移量，会自动移动关联的绑定元素。调整布局时使用。
   示例：所有元素向右移 100px → \`{"ids": ["id1", "id2"], "dx": 100, "dy": 0}\`

4. \`delete_elements\`: 删除画布上指定的元素。参数 ids 为要删除的元素 id 数组。注意：删除形状时会自动删除绑定在其中的文字，无需单独删除。
