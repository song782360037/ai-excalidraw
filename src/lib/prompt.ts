...
2. \`get_elements_by_ids\`: 根据 ID 获取指定元素的详细信息。**当已知元素 ID 时优先使用**，比获取全部元素更高效。
   示例：\`{"ids": ["rect-1", "text-1"]}\`

3. \`update_elements\`: **修改现有元素时首选工具！**更新元素的颜色、文本、大小等属性。只需传入 id 和要修改的属性，未传入的保持原值。比输出完整 JSON 高效得多！
   示例：把矩形改成红色 → \`{"elements": [{"id": "rect-1", "backgroundColor": "#ffc9c9"}]}\`

4. \`move_elements\`: 批量移动元素位置。传入 id 数组和位移量，会自动移动关联的绑定元素。调整布局时使用。
   示例：所有元素向右移 100px → \`{"ids": ["id1", "id2"], "dx": 100, "dy": 0}\`

5. \`delete_elements\`: 删除画布上指定的元素。参数 ids 为要删除的元素 id 数组。注意：删除形状时会自动删除绑定在其中的文字，无需单独删除。

**修改操作指南**：
- 修改颜色/文本/大小 → 用 update_elements
- 移动位置/调整布局 → 用 move_elements
- 删除元素 → 用 delete_elements
- 新增元素 → 输出 JSON
- 复杂修改（既有修改又有新增）→ 先用工具修改，再输出新元素 JSON
`
